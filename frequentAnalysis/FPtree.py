...
if __name__ == "__main__":
    transactions = data.read("Span1.txt")
    frequent_items = _frequent_items(transactions, 2)
    fp_tree, header_table = _build_tree(transactions, frequent_items)

    p = Pattern([], sys.maxsize)
    patterns = FP_growth(fp_tree, header_table, p, 2)
